...
We will perform a hypothesis test on the amount of school shootings during the fall versus the spring semester of the school year. In order to do this, we must define a new variable called ```semester``` with two values, ```spring``` and ```summer```. 

Spring is defined as the months of January through May, when most schools are in session and fall is defined as the months of August through December, when most schools are in session as well. 
...
There seems to be an obvious difference between the amount of school shootings, with the spring semester having an average amount of 28.2 shootings per month. We will perform a hypothesis test on this difference and whether it's significant. 

p1 = average # of shootings per month during the fall
p2 = average # of shootings per month during the spring

Our hypotheses:

H0: the difference is not significant; p1 = p2
HA: the difference is significant; p1 != p2

```{r}
null_dist <- school_shootings %>%
